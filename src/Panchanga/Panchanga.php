...
class Panchanga
{
    ...
    public static function getInstance($anga, $key, array $options = null)
    {
        if (!defined('self::ANGA_'.  strtoupper($anga))) {
            throw new \AstroApp\Panchanga\Exception\InvalidArgumentException("Anga '$anga' does not exist.");
        }
        
        $angaClass = 'AstroApp\\Panchanga\\' . ucfirst($anga) . '\\' . ucfirst($anga);
        
        if (!method_exists($angaClass, 'getInstance')) {
            throw new \AstroApp\Panchanga\Exception\InvalidArgumentException("Instance of '$anga' can not be created.");
        }
