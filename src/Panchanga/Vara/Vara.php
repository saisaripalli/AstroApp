...
namespace AstroApp\Panchanga\Vara;
...
class Vara
{
    ...
    public static function getInstance($key, array $options = null)
    {
        ...
        $varaClass = 'AstroApp\\Panchanga\\Vara\\Object\\' . $key;
     ...
     * 
     * @param string $key
     * @throws \AstroApp\Panchanga\Exception\InvalidArgumentException
     */
    private static function checkVaraKey($key)
    {
        if (!array_key_exists($key, self::$vara)) {
            throw new \AstroApp\Panchanga\Exception\InvalidArgumentException("Vara with the key '$key' does not exist.");
        }
    }
}