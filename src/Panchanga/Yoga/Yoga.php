...
namespace AstroApp\Panchanga\Yoga;
...
class Yoga
{
    ...
    public static function getInstance($key, array $options = null) {
        if (!array_key_exists($key, self::$yoga)) {
            throw new \AstroApp\Panchanga\Exception\InvalidArgumentException("Yoga with the key '$key' does not exist.");
        }
        
        $yogaClass = 'AstroApp\Panchanga\Yoga\Object\Y' . $key;
